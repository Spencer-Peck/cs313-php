...
<h2><?php echo $_GET["name"];?> <div class="text-right"><a href=<?php echo '"'.$add_form_url.'"'; ?> class="btn btn-success " role="button" >Add Score</a></div></h2>            
  <table class="table table-striped" id="games">
    ...
    <?php
    foreach ($db->query('SELECT session_id, winner_name, score, session_description, date FROM game_session WHERE game_id =' . $game_id . ' and user_id ='.$user_id.' ORDER BY date DESC') as $row)
    {
